...
public class CameraActivity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.zoomStepMenu) {
            final Dialog zoomStepDlg = new Dialog(CameraActivity.this);
    ...
    private void takePicture() {
        ...
        try {
            ...
            captureBuilder.set(CaptureRequest.SENSOR_EXPOSURE_TIME, (long) 1e9 / 150);
            ...
            if (x == 1) //if raw is not allowed, do jpeg
            {
                readerListener = new ImageReader.OnImageAvailableListener()
                {
                    //https://www.tabnine.com/code/java/methods/android.media.Image/getPlanes
                    @Override
                    public void onImageAvailable(ImageReader reader) {
                        try (Image image = reader.acquireLatestImage()) //latest image will acquire the latest from the queue and drop older images
                        {
                            ByteBuffer buffer = image.getPlanes()[0].getBuffer(); //getPlanes gets the array of pixel planes for the image contained in the byteBuffer, stores it into buffer
                            byte[] bytes = new byte[buffer.capacity()];
                            buffer.get(bytes);
                            save(bytes); //convert the image into png
                        } catch (IOException e) {
                    ...
                    /* Save the image with PNG format after rotating it */
                    private void save(byte[] bytes) throws IOException {
                        ...
                        try {
                            ...
                            Bitmap rotatedBitmap = rotateImage(originalBitmap, finalRotation); //need to rotate with the original finalRotation saved before
                            output = new FileOutputStream(imageFile);
                            rotatedBitmap.compress(Bitmap.CompressFormat.PNG, 100, output); //from HERE THE output stream gets saved into the actual storage of imageFile
                            output.flush();
                        } finally {
                            if (null != output) {
                                output.close();
                ...
                readerListener = new ImageReader.OnImageAvailableListener()
                {
                    ...
                    public void onImageAvailable(ImageReader reader)
                    {
                        ...
                        try (Image image = reader.acquireLatestImage()) {
                            ...
                            // Convert YUV to RGB
                            final RenderScript rs = RenderScript.create(CameraActivity.this);
            ...
            final CameraCaptureSession.CaptureCallback captureListener = new CameraCaptureSession.CaptureCallback() {
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session, @NonNull CaptureRequest request, @NonNull TotalCaptureResult result) {
                    ...
                    Boast.makeText(CameraActivity.this, "Saved:" + imageFile, Toast.LENGTH_SHORT).show();
                    createCameraPreview();
                }
            };
    ...
    private void zoomIn() {
        ...
        cameraZoom.setZoom(captureRequestBuilder, zoomFactor);
        updatePreview();
        Boast.makeText(CameraActivity.this, "Zoom In: " + zoomFactor, Toast.LENGTH_SHORT).show();
    }
    ...
    protected void zoomOut() {
        ...
        cameraZoom.setZoom(captureRequestBuilder, zoomFactor);
        updatePreview();
        Boast.makeText(CameraActivity.this, "Zoom Out: " + zoomFactor, Toast.LENGTH_SHORT).show();
    }
